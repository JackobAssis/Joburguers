...
(function (global) {
  ...
  /**
   * renderPromocoes
   * - links: array de URLs das postagens públicas do Instagram
   * - containerId: id do grid (default 'promocoes-grid')
   */
  async function renderPromocoes(links, containerId = 'promocoes-grid') {
    if (!Array.isArray(links)) {
      console.error('renderPromocoes espera um array de links do Instagram.');
      return;
    }
    ...
    // Para cada link, criar um card e inserir o blockquote
    links.forEach(link => {
      const card = document.createElement('div');
      card.className = 'promo-card';

      // Criar o blockquote instagram
      const block = createInstagramBlockquote(link);
      card.appendChild(block);

      container.appendChild(card);
    });

    // Carregar (ou reutilizar) o script do Instagram e processar os embeds
